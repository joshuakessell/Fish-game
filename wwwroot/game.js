...
const fishTypes = {
  ...
  6: {
    name: "Lionfish",
    colors: ["#FF4500", "#FF6347", "#8B0000"],
    pattern: "stripes",
    category: "medium",
  },
  7: {
    name: "Parrotfish",
    colors: ["#00CED1", "#48D1CC", "#20B2AA"],
    pattern: "gradient",
    category: "medium",
  },
  8: {
...
async function guestLogin() {
  ...
  try {
    ...
    console.log(
      `Guest login successful: ${playerName} (${userId}) with ${userCredits} credits`,
    );
...
function renderRoomList() {
  ...
  if (roomsData.length === 0) {
    container.innerHTML =
      '<p style="color: #aaa; padding: 20px; grid-column: 1 / -1;">No rooms available...</p>';
    return;
  }
  ...
  try {
    ...
    if (result.success) {
      ...
      try {
        ...
        alert(
          "Game screen error: " + (gameErr.message || JSON.stringify(gameErr)),
        );
        throw gameErr;
      }
    } else {
...
function showSeatSelection(room) {
  ...
  seatSelectionView
    .querySelectorAll(".seat-button[data-seat-index]")
    .forEach((btn) => {
...
function startGame() {
  ...
  if (!canvas) {
    ...
    if (!ctx) {
      ...
      alert(
        "Failed to initialize game canvas. Please try a different browser.",
      );
      return;
    }
...
function showScrollInstruction() {
  ...
  const hasSeenInstruction = localStorage.getItem(
    "oceanking_scroll_instruction_seen",
  );
...
function updateOverlayPositions() {
  ...
  if (betDecreaseBtn && betIncreaseBtn && creditsDisplay) {
    ...
    const offsetY =
      turretPos.y < CANVAS_HEIGHT / 2 ? creditsOffsetY : -creditsOffsetY;
    ...
    decreaseLeft = Math.max(
      0,
      Math.min(window.innerWidth - decreaseWidth, decreaseLeft),
    );
    decreaseTop = Math.max(
      0,
      Math.min(window.innerHeight - decreaseHeight, decreaseTop),
    );

    // Clamp increase button to stay within viewport
    increaseLeft = Math.max(
      0,
      Math.min(window.innerWidth - increaseWidth, increaseLeft),
    );
    increaseTop = Math.max(
      0,
      Math.min(window.innerHeight - increaseHeight, increaseTop),
    );

    // Clamp credits display to stay within viewport
    creditsLeft = Math.max(
      0,
      Math.min(window.innerWidth - creditsWidth, creditsLeft),
    );
    creditsTop = Math.max(
      0,
      Math.min(window.innerHeight - creditsHeight, creditsTop),
    );
...
function showTurretSelection(availableSlots) {
  ...
  function drawSelectionScreen() {
    // Ocean background
    const gradient = selCtx.createLinearGradient(
      0,
      0,
      0,
...
function handleClick(event) {
  ...
  if (length > 0) {
    ...
    if (connection) {
      connection
        .invoke("Fire", playerX, playerY, dirX, dirY)
...
function updateOverlayDisplays() {
  const myPlayer = gameState.players.find(
    (p) => p.playerId === gameState.myPlayerId,
  );
  if (!myPlayer) return;
...
function handleStateDelta(delta) {
  ...
  const oldPlayer = gameState.players.find(
    (p) => p.playerId === gameState.myPlayerId,
  );
  const newPlayer = delta.players.find(
    (p) => p.playerId === gameState.myPlayerId,
  );
  ...
  previousFish.forEach((oldFish) => {
    if (!newFishIds.has(oldFish.fishId)) {
      ...
      if (
        oldPlayer &&
        newPlayer &&
        (newPlayer.credits || 0) > (oldPlayer.credits || 0)
      ) {
  ...
  if (gameState.pendingInteractions.length > 0) {
    const myInteraction = gameState.pendingInteractions.find(
      (i) => i.playerId === gameState.myPlayerId,
    );
    if (myInteraction) {
...
function render() {
  ...
  if (
    currentRect.width !== lastCanvasRect.width ||
  ...
  if (targetedFishId) {
    const targetedFish = gameState.fish.find(
      (f) => f.fishId === targetedFishId,
    );
    if (targetedFish) {
...
function drawCaustics() {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    for (let x = 0; x < canvas.width; x += 30) {
      const y =
        Math.sin((x + animationTime * 80 + i * 100) * 0.015) * 40 +
        Math.cos((x + animationTime * 60) * 0.008) * 30 +
        i * 110;
      if (x === 0) {
        ctx.moveTo(x, y);
      } else {
...
function drawFish(fish) {
  ...
  const finWave =
    Math.sin(animationTime * 4 + fish.fishId.charCodeAt(0) * 0.5) * 0.2;
  const bodyUndulate =
    Math.sin(animationTime * 5 + fish.fishId.charCodeAt(0)) * 0.08;
  ...
  for (let i = 0; i < bodySegments; i++) {
    undulationData.push(
      Math.sin(animationTime * 5 + fish.fishId.charCodeAt(0) + i * 0.8) * 0.12,
    );
  }
...
function drawClownfish(size, colors, anim) {
  ...
  ctx.quadraticCurveTo(
    ...
    anim.tailWag * size,
    -size * 1.5,
    -size * 0.6 + anim.tailWag * size,
  );
  ctx.lineTo(-size * 1.4, 0 + anim.tailWag * size * 0.5);
  ctx.quadraticCurveTo(
    -size * 1.3,
    -anim.tailWag * size * 0.3,
    -size * 1.5,
    size * 0.6 + anim.tailWag * size,
  );
  ctx.closePath();
...
function drawAngelfish(size, colors, anim) {
  ...
  ctx.quadraticCurveTo(
    size * 0.3,
    -size * 2.0 + anim.finWave * size * 0.8,
    size * 0.5,
    -size * 1.3,
  );
  ...
  ctx.quadraticCurveTo(
    -size * 1.2,
    -size * 1.0 + anim.tailWag * size,
    -size * 1.4,
    -size * 0.9 + anim.tailWag * size,
  );
  ctx.quadraticCurveTo(-size * 1.3, anim.tailWag * size * 0.5, -size * 1.2, 0);
  ctx.quadraticCurveTo(
    -size * 1.2,
    size * 1.0 + anim.tailWag * size,
    -size * 1.4,
...
function submitInteraction(interactionId, data) {
  if (connection) {
    connection
      .invoke("SubmitInteraction", interactionId, data)
