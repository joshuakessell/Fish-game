...
function assertPositionEquals(actual: [number, number], expected: [number, number], message: string) {
...
function testLinearPathDeterminism() {
  ...
  for (let i = 0; i < testPoints.length; i++) {
    ...
    assertPositionEquals(pos, [knownPositions[i][0], knownPositions[i][1]], `LinearPath expected position mismatch at t=${testPoints[i]}`);
  }
...
function testCircularPathDeterminism() {
  ...
  assertPositionEquals(posQuarter, [center[0], center[1] + radiusY], 'CircularPath quarter position');
