...
- **Animated Spritesheet System Implemented:** Production-ready fish animation system with rotation-based directional facing and tween-based death effects:
    - **8-Frame Swim Loops:** BootScene loads spritesheets for 8 fish types (clownfish, tetra, butterflyfish, lionfish, triggerfish, hammerhead, manta ray, wave rider) with 8 frames each at 10fps for snappy arcade feel
    - **Rotation-Based Facing:** FishSprite automatically rotates to match velocity angle (Math.atan2) with smooth interpolation (0.15 lerp factor)
    - **Manta Ray Special Handling:** Type 14 uses flipX for left/right facing and clamps rotation to ±45° vertical tilt for realistic gliding appearance
    - **Spiral Death Animation:** 3-spin (1080°) rotation + white flash + scale pop + fade over 1 second using Phaser tweens
    - **Three-Tier Fallback:** Gracefully falls back from spritesheet → static image → fish-0-static when assets missing
    - **Frame Dimensions:** Clownfish/Tetra 72×32, Butterflyfish 80×40, Lionfish 128×56, Triggerfish 120×48, Hammerhead 160×40, Manta Ray 224×96, Wave Rider 112×48
    - **Asset Processing Tools:** Python script (tools/process_fish_sprites.py) created to extract every 3rd frame from 25-frame source sheets, remove black backgrounds, crop water splashes (requires external Python environment)
- **Critical Connection Stability Fix:** Resolved player disconnection issue that blocked all gameplay. The Vite WebSocket proxy was causing `ECONNRESET` errors when the server sent StateDeltas. Implemented direct WebSocket connection to port 8080 bypassing the proxy (`skipNegotiation: true`, `WebSockets` transport in GameState.ts lines 142-149). Moved `onRoomJoined` callback and `roomJoinResolve` to trigger on first StateDelta (lines 275-288) ensuring connection is stable before initializing gameplay. Added SignalR lifecycle event handlers (`onclose`, `onreconnecting`, `onreconnected`) for better debugging. Game now runs successfully with players staying connected for extended sessions.
- **Critical Fish Lifecycle Fixes:** Fixed fish despawn timing by using actual path duration instead of category-based durations (Fish.CreateFish now sets DespawnTick = currentTick + pathData.Duration * 30). Added 200px BOUNDARY_MARGIN to allow off-screen spawning. Fish now properly enter from edges, swim their complete paths, and exit cleanly without freezing or lingering at >110% progress.
- **Starting Balance Update:** Increased starting credits from 1000 to 10,000 for better testing and gameplay experience.
- **Client Death Animations:** Fixed client to only play death animations for PayoutEvents (actual kills), not for natural path completion. Fish now exit silently when their paths complete, death effects only appear when shot.
- **Fire Command Diagnostics:** Added comprehensive logging to HandleFireCommand (Hubs/GameHub.cs) to track fire rate limits, credit validation, coordinate validation, and successful shots with nonces for debugging. Added disconnect exception logging in OnDisconnectedAsync (line 348).
...
- **Duplicate Join Prevention:** Added `isJoiningRoom` guard in GameScene.ts (lines 56, 103-119) to prevent multiple simultaneous join attempts.
